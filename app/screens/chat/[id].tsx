...
export default function ChatDetailScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (!chatError && chatData?.users?.full_name) {
        setDisplayName(chatData.users.full_name);
      }
  ...
  if (!userId) {
    return (
      <SafeAreaView style={styles.container}>
        <Text style={styles.emptyText}>Please log in to view chat</Text>
      </SafeAreaView>
    );
  }
      ...
      <FlashList
        ...
        renderItem={({ item }) => (
          <ChatMessage
            ...
            senderAvatar={item.senderAvatar || avatar}
            image={item.image}
          />
        )}
